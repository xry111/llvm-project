...
LoongArchTargetLowering::LoongArchTargetLowering(const TargetMachine &TM,
  ...
  addRegisterClass(XLenVT, &LoongArch::GPRRegClass);
  ...
  ISD::CondCode FPCCToExpand[] = {ISD::SETOGT, ISD::SETOGE, ISD::SETUGT,
                                  ISD::SETUGE, ISD::SETGT,  ISD::SETGE};

  ISD::NodeType FPOpToExpand[] = {ISD::FSIN,      ISD::FCOS, ISD::FSINCOS,
                                  ISD::FPOW,      ISD::FREM, ISD::FP16_TO_FP,
                                  ISD::FP_TO_FP16};
  ...
  setOperationAction(ISD::FP_TO_UINT, XLenVT, Expand);
  setOperationAction(ISD::STRICT_FP_TO_UINT, XLenVT, Expand);
  setOperationAction(ISD::UINT_TO_FP, XLenVT, Expand);
  setOperationAction(ISD::STRICT_UINT_TO_FP, XLenVT, Expand);
...
bool LoongArchTargetLowering::getTgtMemIntrinsic(IntrinsicInfo &Info,
  ...
  case Intrinsic::loongarch_masked_atomicrmw_max_i32:
  case Intrinsic::loongarch_masked_atomicrmw_min_i32:
  case Intrinsic::loongarch_masked_atomicrmw_umax_i32:
  case Intrinsic::loongarch_masked_atomicrmw_umin_i32:
  case Intrinsic::loongarch_masked_cmpxchg_i32:
...
// On LA32, 64-bit integers are split into their high and low parts and held
// in two different registers, so the trunc is free since the low register can
// just be used.
bool LoongArchTargetLowering::isTruncateFree(Type *SrcTy, Type *DstTy) const {
...
bool LoongArchTargetLowering::isTruncateFree(EVT SrcVT, EVT DstVT) const {
...
bool LoongArchTargetLowering::hasBitPreservingFPLogic(EVT VT) const {
...
SDValue LoongArchTargetLowering::LowerOperation(SDValue Op,
  ...
  case ISD::BITCAST: {
    ...
    if (Op.getValueType() == MVT::f32 && Subtarget.is64Bit() &&
        Subtarget.hasStdExtF()) {
      if (Op0.getValueType() != MVT::i32)
        return SDValue();
      SDValue NewOp0 = DAG.getNode(ISD::ANY_EXTEND, DL, MVT::i64, Op0);
      SDValue FPConv =
          DAG.getNode(LoongArchISD::FMV_W_X_LA64, DL, MVT::f32, NewOp0);
      return FPConv;
    }
    return SDValue();
  }
...
SDValue LoongArchTargetLowering::getAddr(NodeTy *N, SelectionDAG &DAG,
  ...
  if (IsLocal)
    return SDValue(DAG.getMachineNode(LoongArch::PseudoLLA, DL, Ty, Addr), 0);

  SDValue GOT = DAG.getTargetExternalSymbol("_GLOBAL_OFFSET_TABLE_", Ty, 0);
  return SDValue(DAG.getMachineNode(LoongArch::PseudoLA, DL, Ty, Addr, GOT), 0);
}
...
SDValue LoongArchTargetLowering::getStaticTLSAddr(GlobalAddressSDNode *N,
  ...
  unsigned op =
      UseGOT ? LoongArch::PseudoLA_TLS_IE : LoongArch::PseudoLA_TLS_LE;

  SDValue Addr = DAG.getTargetGlobalAddress(GV, DL, Ty, 0, 0);
  SDValue Load;
  if (UseGOT) {
    SDValue GOT = DAG.getTargetExternalSymbol("_GLOBAL_OFFSET_TABLE_", Ty, 0);
    Load = SDValue(DAG.getMachineNode(op, DL, Ty, Addr, GOT), 0);
  } else
    Load = SDValue(DAG.getMachineNode(op, DL, Ty, Addr), 0);

  // Add the thread pointer.
  SDValue TPReg = DAG.getRegister(LoongArch::R2, XLenVT);
...
SDValue LoongArchTargetLowering::getDynamicTLSAddr(GlobalAddressSDNode *N,
  ...
  unsigned op =
      Global ? LoongArch::PseudoLA_TLS_GD : LoongArch::PseudoLA_TLS_LD;
  SDValue Addr = DAG.getTargetGlobalAddress(GV, DL, Ty, 0, 0);
  SDValue GOT = DAG.getTargetExternalSymbol("_GLOBAL_OFFSET_TABLE_", Ty, 0);
  SDValue Load = SDValue(DAG.getMachineNode(op, DL, Ty, Addr, GOT), 0);
...
LoongArchTargetLowering::lowerGlobalTLSAddress(SDValue Op,
  ...
  case TLSModel::LocalDynamic:
    Addr = getDynamicTLSAddr(N, DAG, false);
    break;
  case TLSModel::GeneralDynamic:
    Addr = getDynamicTLSAddr(N, DAG, true);
    break;
  }
...
SDValue LoongArchTargetLowering::lowerVASTART(SDValue Op,
  ...
  LoongArchMachineFunctionInfo *FuncInfo =
...
SDValue LoongArchTargetLowering::lowerFRAMEADDR(SDValue Op,
                                                SelectionDAG &DAG) const {
  const LoongArchRegisterInfo &RI = *Subtarget.getRegisterInfo();
  MachineFunction &MF = DAG.getMachineFunction();
...
static LoongArchISD::NodeType getLoongArchWOpcode(unsigned Opcode) {
  ...
  case ISD::SHL:
    return LoongArchISD::SLLW;
  case ISD::SRA:
    return LoongArchISD::SRAW;
  case ISD::SRL:
    return LoongArchISD::SRLW;
  case ISD::SDIV:
    return LoongArchISD::DIVW;
  case ISD::UDIV:
    return LoongArchISD::DIVUW;
  case ISD::UREM:
    return LoongArchISD::REMUW;
  case ISD::ROTL:
    return LoongArchISD::ROLW;
  case ISD::ROTR:
    return LoongArchISD::RORW;
  }
}

// Converts the given 32-bit operation to a target-specific SelectionDAG node.
// Because i32 isn't a legal type for LA64, these operations would otherwise
...
void LoongArchTargetLowering::ReplaceNodeResults(
  ...
  case ISD::READCYCLECOUNTER: {
    assert(!Subtarget.is64Bit() &&
           "READCYCLECOUNTER only has custom type legalization on LA32");

    SDVTList VTs = DAG.getVTList(MVT::i32, MVT::i32, MVT::Other);
    SDValue RCW =
        DAG.getNode(LoongArchISD::READ_CYCLE_WIDE, DL, VTs, N->getOperand(0));
  ...
  case ISD::BITCAST: {
    ...
    if (N->getValueType(0) == MVT::i32 && Subtarget.is64Bit() &&
      ...
      if (Op0.getValueType() != MVT::f32)
        return;
      SDValue FPConv =
          DAG.getNode(LoongArchISD::FMV_X_ANYEXTW_LA64, DL, MVT::i64, Op0);
...
struct LoongArchBitmanipPat {
  SDValue Op;
  unsigned ShAmt;
  bool IsSHL;

  bool formsPairWith(const LoongArchBitmanipPat &Other) const {
...
SDValue LoongArchTargetLowering::PerformDAGCombine(SDNode *N,
  ...
  default:
    break;
  case LoongArchISD::SplitF64: {
    SDValue Op0 = N->getOperand(0);
    // If the input to SplitF64 is just BuildPairF64 then the operation is
    // redundant. Instead, use BuildPairF64's operands directly.
    if (Op0->getOpcode() == LoongArchISD::BuildPairF64)
    ...
    if (!(Op0.getOpcode() == ISD::FNEG || Op0.getOpcode() == ISD::FABS) ||
      ...
      break;
    SDValue NewSplitF64 =
        DAG.getNode(LoongArchISD::SplitF64, DL,
                    DAG.getVTList(MVT::i32, MVT::i32), Op0.getOperand(0));
  ...
  case LoongArchISD::SLLW:
  case LoongArchISD::SRAW:
  case LoongArchISD::SRLW:
  case LoongArchISD::ROLW:
  case LoongArchISD::RORW: {
  ...
  case LoongArchISD::FMV_X_ANYEXTW_LA64: {
    SDLoc DL(N);
    SDValue Op0 = N->getOperand(0);
    // If the input to FMV_X_ANYEXTW_LA64 is just FMV_W_X_LA64 then the
    // conversion is unnecessary and can be replaced with an ANY_EXTEND
    // of the FMV_W_X_LA64 operand.
    if (Op0->getOpcode() == LoongArchISD::FMV_W_X_LA64) {
    ...
    if (!(Op0.getOpcode() == ISD::FNEG || Op0.getOpcode() == ISD::FABS) ||
        !Op0.getNode()->hasOneUse())
      break;
    SDValue NewFMV = DAG.getNode(LoongArchISD::FMV_X_ANYEXTW_LA64, DL, MVT::i64,
                                 Op0.getOperand(0));
...
bool LoongArchTargetLowering::isDesirableToCommuteWithShift(
  ...
  if (Ty.isScalarInteger() &&
    ...
    if (C1 && C2) {
      ...
      // costs.
      int C1Cost = LoongArchMatInt::getIntMatCost(C1Int, Ty.getSizeInBits(),
                                                  Subtarget.is64Bit());
      int ShiftedC1Cost = LoongArchMatInt::getIntMatCost(
...
bool LoongArchTargetLowering::targetShrinkDemandedConstant(
...
unsigned LoongArchTargetLowering::ComputeNumSignBitsForTargetNode(
...
static MachineBasicBlock *emitReadCycleWidePseudo(MachineInstr &MI,
  ...
  assert(MI.getOpcode() == LoongArch::ReadCycleWide &&
  ...
  BuildMI(LoopMBB, DL, TII->get(LoongArch::RDTIMEH_W), HiReg)
      .addReg(LoongArch::R0);
  BuildMI(LoopMBB, DL, TII->get(LoongArch::RDTIMEL_W), LoReg)
      .addReg(LoongArch::R0);
  BuildMI(LoopMBB, DL, TII->get(LoongArch::RDTIMEH_W), ReadAgainReg)
      .addReg(LoongArch::R0);

  BuildMI(LoopMBB, DL, TII->get(LoongArch::BNE))
      .addReg(HiReg)
      .addReg(ReadAgainReg)
      .addMBB(LoopMBB);
...
static MachineBasicBlock *emitSplitF64Pseudo(MachineInstr &MI,
                                             MachineBasicBlock *BB) {
  assert(MI.getOpcode() == LoongArch::SplitF64Pseudo &&
         "Unexpected instruction");
  ...
  const TargetRegisterClass *SrcRC = &LoongArch::FPR64RegClass;
  int FI = MF.getInfo<LoongArchMachineFunctionInfo>()->getMoveF64FrameIndex(MF);
  ...
  BuildMI(*BB, MI, DL, TII.get(LoongArch::LD_W), LoReg)
      .addFrameIndex(FI)
      .addImm(0)
      .addMemOperand(MMOLo);
  BuildMI(*BB, MI, DL, TII.get(LoongArch::LD_W), HiReg)
      .addFrameIndex(FI)
      .addImm(4)
      .addMemOperand(MMOHi);
...
static MachineBasicBlock *emitBuildPairF64Pseudo(MachineInstr &MI,
  ...
  const TargetRegisterClass *DstRC = &LoongArch::FPR64RegClass;
  int FI = MF.getInfo<LoongArchMachineFunctionInfo>()->getMoveF64FrameIndex(MF);
  ...
  BuildMI(*BB, MI, DL, TII.get(LoongArch::ST_W))
...
MachineBasicBlock *LoongArchTargetLowering::EmitInstrWithCustomInserter(
  ...
  case LoongArch::ReadCycleWide:
    assert(!Subtarget.is64Bit() && "ReadCycleWrite is only to be used on LA32");
    return emitReadCycleWidePseudo(MI, BB);
  case LoongArch::BuildPairF64Pseudo:
    return emitBuildPairF64Pseudo(MI, BB);
  case LoongArch::SplitF64Pseudo:
...
static const MCPhysReg ArgFPR32s[] = {
    LoongArch::F0_F, LoongArch::F1_F, LoongArch::F2_F, LoongArch::F3_F,
    LoongArch::F4_F, LoongArch::F5_F, LoongArch::F6_F, LoongArch::F7_F};
static const MCPhysReg ArgFPR64s[] = {
    LoongArch::F0_D, LoongArch::F1_D, LoongArch::F2_D, LoongArch::F3_D,
    LoongArch::F4_D, LoongArch::F5_D, LoongArch::F6_D, LoongArch::F7_D};

// Pass a 2*XLEN argument that has been split into two XLEN values through
// registers or the stack as necessary.
static bool CC_LoongArchAssign2XLen(unsigned XLen, CCState &State,
...
static bool CC_LoongArch(const DataLayout &DL, LoongArchABI::ABI ABI,
  ...
  // alignment (LA32) or 16-byte alignment (LA64). An aligned register should
  ...
  if (ArgFlags.isSplitEnd() && PendingLocs.size() <= 2) {
    ...
    return CC_LoongArchAssign2XLen(XLen, State, VA, AF, ValNo, ValVT, LocVT,
                                   ArgFlags);
  }

  // Allocate to a register if possible, or else a stack slot.
  Register Reg;
  if (ValVT == MVT::f32 && !UseGPRForF16_F32)
...
void LoongArchTargetLowering::analyzeInputArgs(
  ...
  for (unsigned i = 0; i != NumArgs; ++i) {
    ...
    if (CC_LoongArch(MF.getDataLayout(), ABI, i, ArgVT, ArgVT,
...
static SDValue convertLocVTToValVT(SelectionDAG &DAG, SDValue Val,
  ...
  case CCValAssign::BCvt:
    if (VA.getLocVT().isInteger() && VA.getValVT() == MVT::f16)
      Val = DAG.getNode(LoongArchISD::FMV_H_X, DL, MVT::f16, Val);
    else if (VA.getLocVT() == MVT::i64 && VA.getValVT() == MVT::f32)
      Val = DAG.getNode(LoongArchISD::FMV_W_X_LA64, DL, MVT::f32, Val);
    else
...
static SDValue convertValVTToLocVT(SelectionDAG &DAG, SDValue Val,
  ...
  case CCValAssign::BCvt:
    if (VA.getLocVT().isInteger() && VA.getValVT() == MVT::f16)
      Val = DAG.getNode(LoongArchISD::FMV_X_ANYEXTH, DL, VA.getLocVT(), Val);
    else if (VA.getLocVT() == MVT::i64 && VA.getValVT() == MVT::f32)
      Val = DAG.getNode(LoongArchISD::FMV_X_ANYEXTW_LA64, DL, MVT::i64, Val);
    else
...
static SDValue unpackF64OnLA32DSoftABI(SelectionDAG &DAG, SDValue Chain,
  ...
  return DAG.getNode(LoongArchISD::BuildPairF64, DL, MVT::f64, Lo, Hi);
}

// FastCC has less than 1% performance improvement for some particular
// benchmark. But theoretically, it may has benenfit for some cases.
static bool CC_LoongArch_FastCC(unsigned ValNo, MVT ValVT, MVT LocVT,
                                CCValAssign::LocInfo LocInfo,
                                ISD::ArgFlagsTy ArgFlags, CCState &State) {

  if (LocVT == MVT::i32 || LocVT == MVT::i64) {
    // R12 and R13 (t0 and t1) might be used for save-restore libcall.
    static const MCPhysReg GPRList[] = {
        LoongArch::R4,  LoongArch::R5,  LoongArch::R6,  LoongArch::R7,
        LoongArch::R8,  LoongArch::R9,  LoongArch::R10, LoongArch::R11,
        LoongArch::R14, LoongArch::R15, LoongArch::R16, LoongArch::R17,
        LoongArch::R18, LoongArch::R19, LoongArch::R20};
  ...
  if (LocVT == MVT::f32) {
    static const MCPhysReg FPR32List[] = {
        LoongArch::F10_F, LoongArch::F11_F, LoongArch::F12_F, LoongArch::F13_F,
        LoongArch::F14_F, LoongArch::F15_F, LoongArch::F16_F, LoongArch::F17_F,
        LoongArch::F0_F,  LoongArch::F1_F,  LoongArch::F2_F,  LoongArch::F3_F,
        LoongArch::F4_F,  LoongArch::F5_F,  LoongArch::F6_F,  LoongArch::F7_F,
        LoongArch::F28_F, LoongArch::F29_F, LoongArch::F30_F, LoongArch::F31_F};
  ...
  if (LocVT == MVT::f64) {
    static const MCPhysReg FPR64List[] = {
        LoongArch::F10_D, LoongArch::F11_D, LoongArch::F12_D, LoongArch::F13_D,
        LoongArch::F14_D, LoongArch::F15_D, LoongArch::F16_D, LoongArch::F17_D,
        LoongArch::F0_D,  LoongArch::F1_D,  LoongArch::F2_D,  LoongArch::F3_D,
        LoongArch::F4_D,  LoongArch::F5_D,  LoongArch::F6_D,  LoongArch::F7_D,
        LoongArch::F28_D, LoongArch::F29_D, LoongArch::F30_D, LoongArch::F31_D};
...
static bool CC_LoongArch_GHC(unsigned ValNo, MVT ValVT, MVT LocVT,
  ...
  if (LocVT == MVT::f32) {
    ...
    //                        fs0 ... fs5
    static const MCPhysReg FPR32List[] = {LoongArch::F8_F,  LoongArch::F9_F,
                                          LoongArch::F18_F, LoongArch::F19_F,
                                          LoongArch::F20_F, LoongArch::F21_F};
    if (unsigned Reg = State.AllocateReg(FPR32List)) {
      State.addLoc(CCValAssign::getReg(ValNo, ValVT, Reg, LocVT, LocInfo));
      return false;
    }
  ...
  if (LocVT == MVT::f64) {
    ...
    //                        fs6 ... fs11
    static const MCPhysReg FPR64List[] = {LoongArch::F22_D, LoongArch::F23_D,
                                          LoongArch::F24_D, LoongArch::F25_D,
                                          LoongArch::F26_D, LoongArch::F27_D};
    if (unsigned Reg = State.AllocateReg(FPR64List)) {
      State.addLoc(CCValAssign::getReg(ValNo, ValVT, Reg, LocVT, LocInfo));
      return false;
    }
...
SDValue LoongArchTargetLowering::LowerFormalArguments(
  ...
  case CallingConv::GHC:
    if (!MF.getSubtarget().getFeatureBits()[LoongArch::FeatureStdExtF] ||
        !MF.getSubtarget().getFeatureBits()[LoongArch::FeatureStdExtD])
  ...
  if (CallConv == CallingConv::Fast)
    CCInfo.AnalyzeFormalArguments(Ins, CC_LoongArch_FastCC);
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.getLocVT() == MVT::i32 && VA.getValVT() == MVT::f64)
      ArgValue = unpackF64OnLA32DSoftABI(DAG, Chain, VA, DL);
    else if (VA.isRegLoc())
    ...
    if (VA.getLocInfo() == CCValAssign::Indirect) {
      ...
      // on LA32), we need to load all parts of it here (using the same
      // address).
  ...
  if (IsVarArg) {
    ...
    MachineFrameInfo &MFI = MF.getFrameInfo();
    MachineRegisterInfo &RegInfo = MF.getRegInfo();
    LoongArchMachineFunctionInfo *RVFI =
        MF.getInfo<LoongArchMachineFunctionInfo>();
...
LoongArchTargetLowering::LowerCall(CallLoweringInfo &CLI,
  ...
  if (CallConv == CallingConv::Fast)
    ArgCCInfo.AnalyzeCallOperands(Outs, CC_LoongArch_FastCC);
  else if (CallConv == CallingConv::GHC)
  ...
  for (unsigned i = 0, j = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    bool IsF64OnLA32DSoftABI =
        VA.getLocVT() == MVT::i32 && VA.getValVT() == MVT::f64;
    if (IsF64OnLA32DSoftABI && VA.isRegLoc()) {
      SDValue SplitF64 =
          DAG.getNode(LoongArchISD::SplitF64, DL,
      ...
      if (RegLo == LoongArch::R11) {
        ...
        // Second half of f64 is passed in another GPR.
        assert(RegLo < LoongArch::R20 && "Invalid register pair");
    ...
    // IsF64OnLA32DSoftABI && VA.isMemLoc() is handled below in the same way
    // as any other MemLoc.
  ...
  if (!IsTailCall &&
      MF.getSubtarget<LoongArchSubtarget>().isRegisterReservedByUser(
          LoongArch::R1))
  ...
  if (GlobalAddressSDNode *S = dyn_cast<GlobalAddressSDNode>(Callee)) {
    ...
    unsigned OpFlags = LoongArchII::MO_CALL;
    if (!getTargetMachine().shouldAssumeDSOLocal(*GV->getParent(), GV))
      OpFlags = LoongArchII::MO_PLT;
  ...
  for (auto &VA : RVLocs) {
    ...
    if (VA.getLocVT() == MVT::i32 && VA.getValVT() == MVT::f64) {
      ...
      RetValue = DAG.getNode(LoongArchISD::BuildPairF64, DL, MVT::f64, RetValue,
                             RetValue2);
    }
...
bool LoongArchTargetLowering::CanLowerReturn(
  ...
  for (unsigned i = 0, e = Outs.size(); i != e; ++i) {
    ...
    LoongArchABI::ABI ABI =
        MF.getSubtarget<LoongArchSubtarget>().getTargetABI();
    if (CC_LoongArch(MF.getDataLayout(), ABI, i, VT, VT, CCValAssign::Full,
...
SDValue LoongArchTargetLowering::LowerReturn(
  ...
  for (unsigned i = 0, e = RVLocs.size(); i < e; ++i) {
    ...
    if (VA.getLocVT() == MVT::i32 && VA.getValVT() == MVT::f64) {
      ...
      SDValue SplitF64 = DAG.getNode(LoongArchISD::SplitF64, DL,
  ...
  // Interrupt service routines should use different return instructions.
  // TODO: interrupt attribute is not implemented in LoongArch GCC, so I
  // don't know the semantics of it.
  const Function &Func = DAG.getMachineFunction().getFunction();
  if (Func.hasFnAttribute("interrupt"))
    report_fatal_error("interrupt attribute is not supported yet");

  return DAG.getNode(LoongArchISD::RET_FLAG, DL, MVT::Other, RetOps);
}

void LoongArchTargetLowering::validateCCReservedRegs(
...
#define NODE_NAME_CASE(NODE)                                                   \
  ...
  NODE_NAME_CASE(FMV_W_X_LA64)
  NODE_NAME_CASE(FMV_X_ANYEXTW_LA64)
...
LoongArchTargetLowering::getRegForInlineAsmConstraint(
  ...
  unsigned XRegFromAlias = StringSwitch<unsigned>(Constraint.lower())
                               .Case("{zero}", LoongArch::R0)
                               .Case("{ra}", LoongArch::R1)
                               .Case("{tp}", LoongArch::R2)
                               .Case("{sp}", LoongArch::R3)
                               .Cases("{a0}", "{v0}", LoongArch::R4)
                               .Cases("{a1}", "{v1}", LoongArch::R5)
                               .Case("{a2}", LoongArch::R6)
                               .Case("{a3}", LoongArch::R7)
                               .Case("{a4}", LoongArch::R8)
                               .Case("{a5}", LoongArch::R9)
                               .Case("{a6}", LoongArch::R10)
                               .Case("{a7}", LoongArch::R11)
                               .Case("{t0}", LoongArch::R12)
                               .Case("{t1}", LoongArch::R13)
                               .Case("{t2}", LoongArch::R14)
                               .Case("{t3}", LoongArch::R15)
                               .Case("{t4}", LoongArch::R16)
                               .Case("{t5}", LoongArch::R17)
                               .Case("{t6}", LoongArch::R18)
                               .Case("{t7}", LoongArch::R19)
                               .Case("{t8}", LoongArch::R20)
                               .Cases("{fp}", "{s9}", LoongArch::R22)
                               .Case("{s0}", LoongArch::R23)
                               .Case("{s1}", LoongArch::R24)
                               .Case("{s2}", LoongArch::R25)
                               .Case("{s3}", LoongArch::R26)
                               .Case("{s4}", LoongArch::R27)
                               .Case("{s5}", LoongArch::R28)
                               .Case("{s6}", LoongArch::R29)
                               .Case("{s7}", LoongArch::R30)
                               .Case("{s8}", LoongArch::R31)
                               .Default(LoongArch::NoRegister);
  if (XRegFromAlias != LoongArch::NoRegister)
    return std::make_pair(XRegFromAlias, &LoongArch::GPRRegClass);
  ...
  if (Subtarget.hasStdExtF()) {
    unsigned FReg = StringSwitch<unsigned>(Constraint.lower())
                        .Cases("{f0}", "{fa0}", "{fv0}", LoongArch::F0_F)
                        .Cases("{f1}", "{fa1}", "{fv1}", LoongArch::F1_F)
                        .Cases("{f2}", "{fa2}", LoongArch::F2_F)
                        .Cases("{f3}", "{fa3}", LoongArch::F3_F)
                        .Cases("{f4}", "{fa4}", LoongArch::F4_F)
                        .Cases("{f5}", "{fa5}", LoongArch::F5_F)
                        .Cases("{f6}", "{fa6}", LoongArch::F6_F)
                        .Cases("{f7}", "{fa7}", LoongArch::F7_F)
                        .Cases("{f8}", "{ft0}", LoongArch::F8_F)
                        .Cases("{f9}", "{ft1}", LoongArch::F9_F)
                        .Cases("{f10}", "{ft2}", LoongArch::F10_F)
                        .Cases("{f11}", "{ft3}", LoongArch::F11_F)
                        .Cases("{f12}", "{ft4}", LoongArch::F12_F)
                        .Cases("{f13}", "{ft5}", LoongArch::F13_F)
                        .Cases("{f14}", "{ft6}", LoongArch::F14_F)
                        .Cases("{f15}", "{ft7}", LoongArch::F15_F)
                        .Cases("{f16}", "{ft8}", LoongArch::F16_F)
                        .Cases("{f17}", "{ft9}", LoongArch::F17_F)
                        .Cases("{f18}", "{ft10}", LoongArch::F18_F)
                        .Cases("{f19}", "{ft11}", LoongArch::F19_F)
                        .Cases("{f20}", "{ft12}", LoongArch::F20_F)
                        .Cases("{f21}", "{ft13}", LoongArch::F21_F)
                        .Cases("{f22}", "{ft14}", LoongArch::F22_F)
                        .Cases("{f23}", "{ft15}", LoongArch::F23_F)
                        .Cases("{f24}", "{fs0}", LoongArch::F24_F)
                        .Cases("{f25}", "{fs1}", LoongArch::F25_F)
                        .Cases("{f26}", "{fs2}", LoongArch::F26_F)
                        .Cases("{f27}", "{fs3}", LoongArch::F27_F)
                        .Cases("{f28}", "{fs4}", LoongArch::F28_F)
                        .Cases("{f29}", "{fs5}", LoongArch::F29_F)
                        .Cases("{f30}", "{fs6}", LoongArch::F30_F)
                        .Cases("{f31}", "{fs7}", LoongArch::F31_F)
                        .Default(LoongArch::NoRegister);
    if (FReg != LoongArch::NoRegister) {
      assert(LoongArch::F0_F <= FReg && FReg <= LoongArch::F31_F &&
             "Unknown fp-reg");
      if (Subtarget.hasStdExtD()) {
        unsigned RegNo = FReg - LoongArch::F0_F;
        unsigned DReg = LoongArch::F0_D + RegNo;
...
Instruction *LoongArchTargetLowering::emitLeadingFence(
...
Instruction *LoongArchTargetLowering::emitTrailingFence(
    IRBuilderBase &Builder, Instruction *Inst, AtomicOrdering Ord) const {
...
bool LoongArchTargetLowering::decomposeMulByConstant(LLVMContext &Context,
  ...
  if (VT.isScalarInteger()) {
    // Omit the optimization if the data size exceeds XLen.
    ...
    if (auto *ConstNode = dyn_cast<ConstantSDNode>(C.getNode())) {
      ...
      if ((Imm + 1).isPowerOf2() || (Imm - 1).isPowerOf2() ||
        ...
        return true;
      // Omit the following optimization if the data size >= XLen.
      if (VT.getSizeInBits() >= Subtarget.getXLen())
        return false;
...
namespace llvm {
namespace LoongArchVIntrinsicsTable {

#define GET_LoongArchVIntrinsicsTable_IMPL
#include "LoongArchGenSearchableTables.inc"

} // namespace LoongArchVIntrinsicsTable

namespace LoongArchZvlssegTable {

#define GET_LoongArchZvlssegTable_IMPL
#include "LoongArchGenSearchableTables.inc"
